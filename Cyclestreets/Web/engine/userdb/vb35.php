<?php
/*********************************************************
 * $HeadURL: http://www.maidenfans.com/svn/rwdownload_new/TRUNK/engine/userdb/vb35.php $
 * $Revision: 50 $
...
class vb35database
{
	...
	function getDetails($configpath)
	{
        ...
        else
        {
    		...
    		$dbinfo = array("sqlhost" => $config['MasterServer']['servername'],
    						"sqlusername" => $config['MasterServer']['username'],
    						"sqlpassword" => $config['MasterServer']['password'],
    						"sqldatabase" => $config['Database']['dbname'],
    						"sql_tbl_prefix" => $config['Database']['tableprefix']);

    		$this->parent->userdb = new mysql($dbinfo);

    		$this->parent->mem_table = $config['Database']['tableprefix']."user";
    		$this->parent->mem_gtable = $config['Database']['tableprefix']."usergroup";
        }
		return true;
	}
