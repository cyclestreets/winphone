<?php
/*********************************************************
 * $HeadURL: http://www.maidenfans.com/svn/rwdownload_new/TRUNK/engine/xml.php $
 * $Revision: 50 $
...
{    
    ...
    function GetChildFromPath($path=0)
    {
        $result = array();
        for($element=0;$element<$this->structure[$path]["Elements"];$element++)
        {
            if(GetType($this->structure[$path.",$element"])=="array")
            {
                $children = array();
                $thiselement = array();
                if ( $this->structure[$path.",$element"]["Elements"] > 0 )
                {   
                    $children = $this->GetChildFromPath($path.",$element");
                }
                $thiselement = $this->structure[$path.",$element"];
                $result['children'][] = array_merge($thiselement, $children);
                $result['path'] = $path;
            }
        }
        return $result;
    ...
    function GetChildElementsFromTag($tagname, $path=0)
    {
        if ( $this->structure[$path]["Tag"] == $tagname )
        {
            $result = $this->GetChildFromPath($path);
            return $result;
        }
        if(GetType($this->structure[$path])=="array")
        {
            for($element=0;$element<$this->structure[$path]["Elements"];$element++)
            {
                $res = $this->GetChildElementsFromTag($tagname,$path.",$element");
                if ( $res )
                    return $res;
            }
        }
    ...
    function GetDataFromTag($tagname, $path=0)
    {
        if ( $this->structure[$path]["Tag"] == $tagname )
        {
        ...
        if(GetType($this->structure[$path])=="array")
        {
            for($element=0;$element<$this->structure[$path]["Elements"];$element++)
            {
                $res = $this->GetDataFromTag($tagname,$path.",$element");
                if ( $res )
                    return $res;
            }
        }
...
class xml_parser_class
{
    ...
    {
        if(!strcmp($this->error,""))
            $this->parser_handler->EndElement($this,trim($name));
    }
